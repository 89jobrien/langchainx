...
impl Tool for RustqualTool {
    ...
    async fn run(&self, input: Value) -> Result<String, ToolError> {
        let parsed: RustqualInput = serde_json::from_value(input)
            .map_err(|e| ToolError::InvalidInput(e.to_string()))?;

        // TODO(#81): path traversal -- join allows ../../ escapes; canonicalize and
        //       verify result is under base_dir
        let target = self.base_dir.join(&parsed.path);
