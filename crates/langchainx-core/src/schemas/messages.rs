//! Chat message roles, content, images, and tool-call metadata.
...
pub enum MessageType {
    #[default]
    #[serde(rename = "system")]
    /// Instructions that establish model behavior or context.
    SystemMessage,
    #[serde(rename = "ai")]
...
/// An image reference included in a multimodal message.
...
/// A chat message with optional tool-call and image data.
///
/// # Usage
/// ```rust,ignore
...
pub struct Message {
    ...
    /// Optional identifier, used for tool responses and provider messages.
    pub id: Option<String>,
    /// Provider-compatible tool calls requested by the assistant.
...
impl Message {
    ...
    /// Creates a human message containing text.
    ...
    /// Creates a human message containing images and no text.
    ...
    /// Creates a system message containing text.
    ...
    /// Creates an AI message containing text.
    ...
    /// Creates a tool response with the corresponding tool-call identifier.
    ...
    /// Attaches provider-compatible tool-call data to the message.
    pub fn with_tool_calls(mut self, tool_calls: Value) -> Self {
        self.tool_calls = Some(tool_calls);
        self
    }

    /// Deserializes a JSON value into a list of messages.
    pub fn messages_from_value(value: &Value) -> Result<Vec<Message>, serde_json::error::Error> {
        serde_json::from_value(value.clone())
    }

    /// Formats messages as newline-separated `MessageType: content` lines.
